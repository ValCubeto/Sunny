...
use crate::eval::tokenize::{
  keywords::Keyword as Kw, tokens::{ Operator as Op, Token as Tk, TokenIter }, Position
};

pub fn parse_mod(tokens: Vec<(Position, Tk)>) -> Vec<Entity> {
  let mut items = Vec::new();
  let mut tokens = TokenIter::new(tokens.iter().peekmore());
  // Allow to change the metadata for all following items
  ...
  Some(match token {
    Tk::Keyword(Kw::Mod) => {
      match tokens.next_token() {
        Tk::Ident(name) => syntax_err!("modules not yet implemented"),
    ...
    Tk::Keyword(Kw::ArgList) => {
      syntax_err!("argument structs not yet implemented");
    }
    Tk::Keyword(Kw::Class) => {
      syntax_err!("classes not yet implemented");
    }
    Tk::Keyword(Kw::Interface) => {
      syntax_err!("interfaces not yet implemented");
    }
    Tk::Keyword(Kw::Impl) => {
    ...
    _ => syntax_err!("unexpected {}", token.to_string())
  })
}
